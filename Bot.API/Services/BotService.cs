...
public class BotService
{
    ...
    public BotService(string botToken, ILogger<BotService> logger)
    {
        ...
        _channelId = "@dachatop_uzbekistan"; // Ensure channel ID starts with '@'
        _logger = logger;
    }
    ...
    private async Task HandleCheckMembershipCallbackAsync(ITelegramBotClient botClient, CallbackQuery callbackQuery, CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            await SendWelcomeMessageAsync(botClient, callbackQuery.Message!.Chat.Id, cancellationToken);
        }
    }

    private async Task SendWelcomeMessageAsync(ITelegramBotClient botClient, long chatId, CancellationToken cancellationToken)
    {
        var photoUrl = "https://static.wixstatic.com/media/b80424_6ef0f3d7568d49baa059b228cdc68cd3~mv2.jpeg/v1/fit/w_2500,h_1330,al_c/b80424_6ef0f3d7568d49baa059b228cdc68cd3~mv2.jpeg";
        ...
        try
        {
            ...
            {
                InlineKeyboardButton.WithWebApp("Web App", new WebAppInfo() { Url = "https://dachacentre.netlify.app/" }),
            });

            await botClient.SendPhotoAsync(
                chatId: chatId,
                photo: new InputFileUrl(photoUrl),
                caption: "Hush kelibsiz, botdan foydalangan holda istalgancha dachalarni izlab toping va oilaviy, do'stlar bilan roxatlaning",
